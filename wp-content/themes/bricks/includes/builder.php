...
class Builder {
	...
	public function enqueue_scripts() {
		...
		// Get control options to ensure filter 'bricks/setup/control_options' ran (e.g. 'queryTypes', custom user control options, etc. (@since 1.5.5)
	...
	public static function builder_data( $post_id ) {
		...
		$template_preview_post_id = $template_preview_post_id || $post_id;
	...
	public static function query_content_type_for_elements_html( $elements ) {
		...
		foreach ( $elements as $element ) {
			...
			if (
				$element_name === 'code' || // Code element: to prevent critical errors with code execution enabled when loading the builder (@since 1.5.7)
				$element_name === 'template' // Template element: to render template inline CSS in builder (@since 1.5.7)
			) {
				continue;
			}
	...
	public static function is_builder_call() {
		// STEP: Builder AJAX call: Check data for 'bricks-is-builder' (@since 1.5.5)
