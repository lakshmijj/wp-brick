...
class Element_Carousel extends Element {
	...
	public function render() {
		...
		// Used in case link_to = lightbox. It needs to be random because if the carousel is placed inside of a loop the lightbox fails (@since 1.5.1)
		$lightbox_id = Helpers::generate_random_id( false );
				...
				if ( $image_url ) {
					...
					// Use background image
						...
						if ( $type === 'media' && isset( $settings['imageLightbox'] ) ) {
							...
							$this->set_attribute( "image-$item_index", 'data-bricks-lightbox-id', $lightbox_id );
