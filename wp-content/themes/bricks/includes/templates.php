...
class Templates {
	...
	public function render_shortcode( $attributes = [] ) {
		 ...
		 *
		 * Including .brx-popup wrapper, .brx-popup-content, etc.
		 *
		 * @since 1.6
		 */
		if ( self::get_template_type( $template_id ) === 'popup' ) {
			...
			Popups::render_popups( [ $template_id ] );
	...
	public static function generate_inline_css( $template_id, $elements ) {
		...
		if ( ! empty( Assets::$inline_css[ "template_$template_id" ] ) ) {
	...
	public function import_template() {
		...
		foreach ( $templates as $template_data ) {
			...
			foreach ( $template_global_classes as $template_global_class ) {
				// Skip if class with same 'id' exists locally
				$class_index = array_search( $template_global_class['id'], array_column( $global_classes, 'id' ) );
				...
				$class_index = array_search( $template_global_class['name'], array_column( $global_classes, 'name' ) );

				if ( $class_index !== false ) {
					$map_classes[ $template_global_class['id'] ] = $global_classes[ $class_index ]['id'];

					continue;

					// NOTE: No longer in use @since 1.5.1 (replaced my 'mapClasses' logic)
					// Append '__imported' to template class name to distinguish it from local class name
					// $template_global_class['name'] = "{$template_global_class['name']}__imported";
				}

				// Update global classes in database
				$global_classes[] = $template_global_class;
			}
