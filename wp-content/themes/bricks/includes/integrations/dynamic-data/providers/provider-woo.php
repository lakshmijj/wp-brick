	...
	public function get_tag_value( $tag, $post, $args, $context ) {
		...
		switch ( $render ) {
			...
			case 'cart_product_name':
				...
				if ( $loop_object_type === 'wooCart' ) {
					...
					if ( ! $product_permalink ) {
						$value = wp_kses_post( apply_filters( 'woocommerce_cart_item_name', $_product->get_name(), $loop_object, $cart_item_key ) . '&nbsp;' );
					} else {
					...
					if ( $_product->backorders_require_notification() && $_product->is_on_backorder( $loop_object['quantity'] ) ) {
						$value .= wp_kses_post( apply_filters( 'woocommerce_cart_item_backorder_notification', '<p class="backorder_notification">' . esc_html__( 'Available on backorder', 'woocommerce' ) . '</p>', $product_id ) );
					}
				}
			...
			case 'cart_remove_link':
				...
				$value = $product && $cart_item_key ? apply_filters(
