...
class Provider_Wp extends Base {
	...
	public function get_post_meta_keys() {
		...
		// Only load WordPress custom fields in builder (@since 1.5.5)
		if ( ! bricks_is_builder() ) {
			return $list;
		}

		// Return: User doesn't want to retrieve default WP custom fields in builder (@since 1.5.5)
		if ( \Bricks\Database::get_setting( 'builderDisableWpCustomFields', false ) ) {
			return $list;
		}
	...
	public function get_tag_value( $tag, $post, $args, $context ) {
		...
		switch ( $render ) {
			...
			case 'featured_image_tag':
				...
				// Get the WooCommerce placeholder image if featured image is empty (@since 1.5.1)
		...
		if ( strpos( $callback, '(' ) !== false ) {
			...
			if ( ! empty( $parts[1] ) ) {
				...
				foreach ( str_split( $parts[1] ) as $char ) {
					...
					if ( ! $in_quote && $char == ',' ) {
						// Chunk is not empty, add it as argument
						if ( ! empty( $chunk ) ) {
							$args[] = $chunk;
			...
			// Maybe we still have one argument (no single quote or comma in the end)
			if ( ! empty( $chunk ) ) {
				$args[] = $chunk;
			}
