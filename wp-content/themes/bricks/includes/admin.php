...
class Admin {
	...
	public function save_settings() {
		...
		foreach ( $settings as $key => $value ) {
			...
			if ( is_string( $value ) && strpos( $value, 'xxxx' ) !== false ) {
				...
				// Preserve backslashes in custom code (@since 1.5.1)
	...
	public function admin_notice_regenerate_css_files() {
		// Return: No access to Bricks settings
		if ( ! current_user_can( 'manage_options' ) ) {
			return;
		}
		...
		// Return: CSS files have been generated for this theme version (options db entry matches installed theme version)
