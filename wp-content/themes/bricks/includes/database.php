...
class Database {
	...
	/**
	 * Checks if header or footer is disabled (via page settings) for the current context
	 *
	...
	public static function is_template_disabled( $template_type ) {
		...
		$template_id = isset( self::$active_templates['content'] ) ? self::$active_templates['content'] : 0;

		// Post is rendered through a Bricks template, but post also has Bricks data
		if ( $template_id && $template_id !== self::$page_data['original_post_id'] ) {
			$page_settings = get_post_meta( self::$page_data['original_post_id'], BRICKS_DB_PAGE_SETTINGS, true );

			$is_disabled = isset( $page_settings[ $setting_key ] );
		}

		return $is_disabled || isset( self::$page_settings[ $setting_key ] );
	}
