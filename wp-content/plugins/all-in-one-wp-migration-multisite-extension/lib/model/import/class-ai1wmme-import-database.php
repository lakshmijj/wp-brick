...
class Ai1wmme_Import_Database {
	...
	public static function execute( $params ) {
		...
		// Retention settings
		$backups_total_size  = get_option( 'ai1wmme_total' );
		$backups_total_files = get_option( 'ai1wmme_backups' );
		$backups_total_days  = get_option( 'ai1wmme_days' );
		...
		if ( empty( $multisite['Network'] ) ) {
			...
			// Set main table prefixes (preserve mainsite prefix)
		...
		if ( ! get_option( AI1WM_ACTIVE_PLUGINS ) ) {
		...
		update_option( 'ai1wmme_total', $backups_total_size );
		update_option( 'ai1wmme_backups', $backups_total_files );
		update_option( 'ai1wmme_days', $backups_total_days );
